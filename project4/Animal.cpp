...
istream& operator>>(istream& in, Animal& animal)
{
    ...
    //Capitalize the type and gender
    typeTemp = toupper(typeTemp);
    genderTemp = toupper(genderTemp);
...
ostream& operator<<(ostream& out, const Animal& animal)
{
    ...
    out << left << setfill(' ') << setw(15) << animal.name << setw(15) << typeWord << setw(15) << genderWord << endl;
...
/*
    Function Name: operator<
    Description:   Compares the passed animal to the current animal object
                   by name
    Parameters:    Animal - animal: animal object in the Animal class
    Return Value:  bool - true if the current name is less than the passed
                          animal
                          false if not
*/
bool Animal::operator<(const Animal& animal)
{
    //Compare the names of the two animals
    if(name < animal.name)
    {
        return true;
    }// end if
